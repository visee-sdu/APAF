...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    best_val = {'val_unseen': {"spl": 0., "sr": 0., "nDTW": 0., "state":""}}
    if args.dataset == 'r4r':
        best_val = {'val_unseen_sampled': {"spl": 0., "sr": 0., "nDTW": 0., "state":""}}
        ...
        for env_name, env in val_envs.items():
            ...
            if default_gpu:
                ...
                if env_name in best_val:
                    ...
                    elif args.expert_policy == 'ndtw':
                        if score_summary['ndtw'] >= best_val[env_name]['ndtw']:
                            best_val[env_name]['spl'] = score_summary['spl']
                            best_val[env_name]['sr'] = score_summary['sr']
                            best_val[env_name]['nDTW'] = score_summary['nDTW']
                            best_val[env_name]['state'] = 'Iter %d %s' % (iter, loss_str)
