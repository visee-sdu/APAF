...
class BertOutAttention(nn.Module):
    ...
    def forward(self, hidden_states, context, attention_mask=None):
        # hidden_states: torch.Size([6, 13, 768]), context: torch.Size([6, 104, 768])
        mixed_query_layer = self.query(hidden_states)
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)  # torch.Size([6, 12, 13, 104])
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
        if attention_mask is not None:
            attention_scores = attention_scores + attention_mask
        
        # from r2r.agent import flag, toggle_flag
        # if flag:
        #     toggle_flag()
...
class ObjectEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.object_encoder = create_transformer_encoder(config, config.num_pano_layers, norm=True)
...
class ActionEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.action_encoder = create_transformer_encoder(config, config.num_pano_layers, norm=True)
...
class BackHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.back_linear = nn.Linear(config.hidden_size, config.hidden_size)
        self.back_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
    def forward(self, txt_embeds):
        return self.back_layer_norm(self.back_linear(txt_embeds))    
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_r2r_back(self, txt_embeds):
        return self.back_head(txt_embeds)
