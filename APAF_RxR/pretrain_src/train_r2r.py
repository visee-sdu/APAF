...
from data.tasks import (
    ...
    MaskSemDataset, masksem_collate)
...
def create_dataloaders(
    ...
    for k, task_name in enumerate(data_cfg.tasks):
        if task_name == 'mlm':
            task_dataset = MlmDataset(nav_db, tok, end_vp_pos_ratio=0.75)
            task_collate_fn = mlm_collate
        elif task_name == 'sap':
            task_dataset = SapDataset(nav_db, tok, end_vp_pos_ratio=0.2)
            task_collate_fn = sap_collate
        elif task_name == 'sem':
            task_dataset = SemDataset(nav_db, tok, end_vp_pos_ratio=0.2)
            task_collate_fn = sem_collate
        elif task_name == 'masksem':
            task_dataset = MaskSemDataset(nav_db, tok, opts.bev_mrc_mask_prob, end_vp_pos_ratio=0.2)
...
def validate_sem(model, val_loader):
    ...
    outputs = sum(all_gather(outputs), []); outputs = torch.cat(outputs, dim=0)
    targets = sum(all_gather(targets), []); targets = torch.cat(targets, dim=0)
