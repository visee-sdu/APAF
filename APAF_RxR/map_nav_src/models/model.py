...
class VLNBert(nn.Module):
    ...
    def forward(self, mode, batch):
        ...
        if mode == 'language':            
            ...
            obj_embeds, object_masks = self.vln_bert(mode, batch)
            return obj_embeds, object_masks
        elif mode == 'action':
            action_embeds, action_masks = self.vln_bert(mode, batch)
            return action_embeds, action_masks
        elif mode == 'cocoop':
            txt_features = self.vln_bert(mode, batch)
            return txt_features
        elif mode == 'r2r_back':
            txt_embeds = self.vln_bert(mode, batch)
            return txt_embeds
