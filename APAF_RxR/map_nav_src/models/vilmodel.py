...
from models.ops import create_transformer_encoder
from models.ops import extend_neg_masks, gen_seq_masks, pad_tensors_wgrad
...
class PromptEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, step_ids, token_type_ids=None, position_ids=None):
        seq_length = input_ids.size(1)
        if position_ids is None:
            position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
            position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
        ...
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
        step_embeddings = self.step_embeddings(step_ids)

        embeddings = words_embeddings + position_embeddings + token_type_embeddings + step_embeddings
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class PhaseEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_phase_layers = 2
        self.update_lang_bert = config.update_lang_bert

        self.layer = nn.ModuleList(
            [BertLayer(config) for _ in range(self.num_phase_layers)]
        )
        if not self.update_lang_bert:
            for name, param in self.layer.named_parameters():
                param.requires_grad = False
...
class LocalBEVEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.bev_dim = config.bev_dim
        self.bev_fts_embeddings = nn.Sequential(
            nn.Linear(768, config.hidden_size),
            BertLayerNorm(config.hidden_size, eps=1e-12)
        )
        self.bev_pos_embeddings = nn.Sequential(
            nn.Linear(3 + 7, config.hidden_size),
            BertLayerNorm(config.hidden_size, eps=1e-12)
        )
    ...
    def bev_input_embedding(self, bev_fts, bev_pos_fts, bev_nav_masks):
        bev_embeds = self.bev_fts_embeddings(bev_fts) + \
                     self.bev_pos_embeddings(bev_pos_fts) + \
                     self.nav_type_embedding(bev_nav_masks.long())
        return bev_embeds
    ...
    def forward(
        ...
        bev_embeds = self.bev_input_embedding(bev_fts, bev_pos_fts, bev_nav_masks)
        if obj_embeds is not None:
            bev_obj_embeds = torch.cat([bev_embeds, obj_embeds], dim=1)
            bev_obj_masks = torch.cat([bev_masks, obj_masks], dim=1)
        else:
            bev_obj_embeds = bev_embeds
            bev_obj_masks = bev_masks

        bev_obj_embeds = self.encoder(txt_embeds, txt_masks, bev_obj_embeds, bev_obj_masks)
        bev_embeds = bev_obj_embeds[:, :self.bev_dim*self.bev_dim]
        if obj_embeds is not None:
            obj_embeds = bev_obj_embeds[:, self.bev_dim*self.bev_dim:]
        else:
            obj_embeds = None

        return bev_embeds, obj_embeds
...
class ObjectEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.obj_linear = nn.Linear(config.object_dim, config.hidden_size)
        self.obj_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.loc_linear = nn.Linear(4, config.hidden_size)
        self.loc_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.feature_type_embedding = nn.Embedding(2, config.hidden_size)
        
        self.object_encoder = create_transformer_encoder(config, 2, norm=True)
        
        self.layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        

    def forward(self, obj_img_fts, obj_loc_fts, obj_feature_types, object_lens):
        obj_embeds = self.obj_layer_norm(self.obj_linear(obj_img_fts))  # (batch_size, max_objects, dim_ft)
        all_embeds = obj_embeds + \
                    self.loc_layer_norm(self.loc_linear(obj_loc_fts)) + \
                    self.feature_type_embedding(obj_feature_types)
        all_embeds = self.layer_norm(all_embeds)
        all_embeds = self.dropout(all_embeds)
        
        obj_masks = gen_seq_masks(object_lens)
        all_embeds = self.object_encoder(all_embeds, src_key_padding_mask=obj_masks.logical_not())
        return all_embeds, obj_masks
    
class ActionEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.action_heading_embedding = nn.Embedding(360, config.hidden_size)
        self.action_elevation_embedding = nn.Embedding(60, config.hidden_size)
        
        self.layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.action_encoder = create_transformer_encoder(config, 1, norm=True)
    def forward(self, action_heading_ids, action_elevation_ids, action_lens):
        action_embeds = self.action_heading_embedding(action_heading_ids) + self.action_elevation_embedding(action_elevation_ids)
        action_embeds = self.layer_norm(action_embeds)
        action_embeds = self.dropout(action_embeds)
        action_masks = gen_seq_masks(action_lens)
        action_embeds = self.action_encoder(action_embeds, src_key_padding_mask=action_masks.logical_not())
        return action_embeds, action_masks
...
class CrossAttention(nn.Module):
    def __init__(self, config):
        ...
        self.step_embeddings = nn.Embedding(config.max_action_steps, config.hidden_size)
        self.type_embeddings = nn.Embedding(4, config.hidden_size)

    def forward(self, txt_embeds, txt_masks, img_embeds, img_masks, txt_step_ids, vision_step_ids, txt_type_ids, vision_type_ids):
        extended_txt_masks = extend_neg_masks(txt_masks)
        extended_img_masks = extend_neg_masks(img_masks) # (N, 1(H), 1(L_q), L_v)
        
        txt_embeds = txt_embeds + self.step_embeddings(txt_step_ids) + self.type_embeddings(txt_type_ids)
        img_embeds = img_embeds + self.step_embeddings(vision_step_ids) + self.type_embeddings(vision_type_ids)
        
        lang_att_output = self.visual_attention(
            txt_embeds, img_embeds, ctx_att_mask=extended_img_masks
        )[0]
        lang_att_output = self.lang_self_att(
            lang_att_output, extended_txt_masks
        )[0]
        lang_inter_output = self.lang_inter(lang_att_output)
        lang_output = self.lang_output(lang_inter_output, lang_att_output)
        return lang_output  
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.local_encoder = LocalBEVEncoder(config)
        ...
        self.object_encoder = ObjectEncoder(config)
    ...
    def forward_navigation_per_step(
        ...
        bev_fts, bev_pos_fts, bev_masks, bev_nav_masks, bev_cand_idxs, bev_cand_vpids,
        obj_embeds, obj_masks,
    ):
        ...
        # local branch
        bev_embeds, obj_embeds = self.local_encoder(txt_embeds, txt_masks, 
                                                    bev_fts, bev_pos_fts, bev_masks, bev_nav_masks,
                                                    obj_embeds, obj_masks)
        ...
        if self.sap_fuse_linear is None:
            ...
            CENTER = (self.bev_dim * self.bev_dim - 1) // 2
        ...
        for i in range(batch_size):
            ...
            tmp = {}
            bw_logits = 0
            for j, cand_vpid in enumerate(bev_cand_vpids[i]):
                if j > 0:
                    if cand_vpid in visited_nodes:
                        bw_logits += local_logits[i, j]     # visited, accumulate back logits
                    else:
                        tmp[cand_vpid] = local_logits[i, j] # unvisited, vp logits
            for j, vp in enumerate(gmap_vpids[i]):
                if j > 0 and vp not in visited_nodes:
                    if vp in tmp:
                        fused_logits[i, j] += tmp[vp]       # if unvisited, fuse vp logits
                    else:
                        fused_logits[i, j] += bw_logits     # if visited, fuse back logits
    ...
    def forward_object_per_step(self, obj_img_fts, obj_loc_fts, obj_feature_types, object_lens):
        obj_embeds, obj_masks = self.object_encoder(
            obj_img_fts, obj_loc_fts, obj_feature_types, object_lens
        )
        return obj_embeds, obj_masks
    
    
    def forward_action_per_step(self, action_heading_ids, action_elevation_ids, action_lens):
        action_embeds, action_masks = self.action_encoder(action_heading_ids, action_elevation_ids, action_lens)
        return action_embeds, action_masks
    
    def forward_cocoop(self, txt_embeds, txt_masks, img_embeds, img_masks, txt_step_ids, vision_step_ids, txt_type_ids, vision_type_ids):
        embeds = self.crossattention(txt_embeds, txt_masks, img_embeds, img_masks, txt_step_ids,vision_step_ids, txt_type_ids, vision_type_ids)
        return embeds
        ...
        if mode == 'language':
            ...
            return pano_embeds, pano_masks
             ...
             return self.forward_navigation_per_step(
                ...
                # bev
                batch['bev_fts'], batch['bev_pos_fts'], 
                batch['bev_masks'], batch['bev_nav_masks'], 
                batch['bev_cand_idxs'], batch['bev_cand_vpids'],
                # obj
                batch['obj_embeds'], batch['obj_masks'],
            )
        elif mode == 'object':
            obj_embeds, obj_masks = self.forward_object_per_step(
                batch['object_img_fts'], batch['object_loc_fts'], batch['object_feature_types'], 
                batch['object_lens']
            )
            return obj_embeds, obj_masks
        elif mode == 'action':
            action_embeds, action_masks = self.forward_action_per_step(
                batch['action_heading_ids'], batch['action_elevation_ids'], batch['action_lens']
            )
            return action_embeds, action_masks
        elif mode == 'cocoop':
            prompt_embeds = self.forward_cocoop(batch['txt_features'], batch['txt_masks'], batch['vision_features'], batch['vision_masks'],
                                                batch['txt_step_ids'], batch['vision_step_ids'], batch['txt_type_ids'], batch['vision_type_ids'])
