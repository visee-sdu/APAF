...
class R2RNavBatch(object):
    ...
    def _load_nav_graphs(self):
        ...
        self.graphs, self.scanvp_position = load_nav_graphs(self.connectivity_dir, self.scans)
        self.shortest_paths = {}
    ...
    def make_candidate(self, feature, state):
        ...
        if long_id not in self.buffered_state_dict:
            for i, loc in enumerate(state.navigableLocations[1:]):
                ...
                pointId = nearest_anchor(norm_elevation, ANCHOR_E) * 12 + nearest_anchor(norm_heading, ANCHOR_H)  # 数字，最近的视角id
