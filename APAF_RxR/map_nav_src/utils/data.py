...
class ImageFeaturesDB(object):
    def __init__(self, img_ft_file, image_feat_size=None, in_memory=True):
        self.image_feat_size = image_feat_size
        self.img_ft_file = img_ft_file
        self.in_memory = in_memory
        self.dtype = np.uint8 if 'semantic' in img_ft_file else np.float32
        self._feature_store = {}
    ...
    def get_image_feature(self, scan, viewpoint):
        ...
        if key in self._feature_store:
            ...
            with h5py.File(self.img_ft_file, 'r') as f:
                if self.image_feat_size is not None:
                    ft = f[key][...][:, :self.image_feat_size].astype(self.dtype)
                else:
                    ft = f[key][...].astype(self.dtype)
                if self.in_memory:
